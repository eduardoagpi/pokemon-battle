import { useLobbyStatusViewController } from './LobbyStatusViewController';

export function LobbyStatus() {
    const { uiState, actions } = useLobbyStatusViewController();

    return (
        <section className="w-full max-w-[900px] flex flex-col items-center gap-12 animate-fade-in-slow">
            <div className="w-full flex justify-start">
                <button
                    onClick={actions.onClickedGoBack}
            ...
            <div className="text-center glass-panel w-full p-8">
                <h2 className="text-[2.2rem] mb-2 text-primary">¡Hola, {uiState.nickname}!</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-8 w-full">
                {uiState.assignedPokemons.map((pokemon) => (
